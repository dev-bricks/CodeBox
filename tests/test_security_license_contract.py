"""Automated security, dependency floor, and third-party license contract tests for CodeBox."""
...
def test_dependency_vulnerability_floors() -> None:
    ...
    # paramiko >= 5.0.0 eliminates SHA-1 weakness GHSA-r374-rxx8-8654
    assert "paramiko>=5.0.0" in pyproject_text, "pyproject.toml remote dependencies must require paramiko>=5.0.0"
    ...
    # Pillow >= 12.3.0 protects icon & asset tests against known Pillow CVEs
...
def test_no_hardcoded_user_paths_in_python_code() -> None:
    ...
    for sub in ["core", "features", "ui", "utils", "tests", "plugins"]:
        sub_path = ROOT / sub
        if sub_path.is_dir():
            python_files.extend(sub_path.rglob("*.py"))

    assert len(python_files) > 20, f"Expected >20 Python files to scan, found {len(python_files)}"
...
def test_security_policy_bilingual_and_sla() -> None:
    ...
    assert "security@dev-bricks.org" in content, "security@dev-bricks.org contact missing"
    ...
    assert "advisories/new" in content, "Private vulnerability reporting link missing"


def test_local_first_and_offline_invariants() -> None:
    """Verify CodeBox preserves offline, zero-egress, and non-elevation guarantees."""
    sec_file = ROOT / "SECURITY.md"
    assert sec_file.is_file()
    content = sec_file.read_text(encoding="utf-8")

    assert "100% Offline" in content or "Offline" in content
    assert "Zero-Egress" in content or "zero-egress" in content
    assert "Non-Elevation" in content or "Keine Administratorrechte" in content
    ...
    prod_files = []
    for sub in ["core", "features", "ui", "utils"]:
        sub_path = ROOT / sub
        if sub_path.is_dir():
